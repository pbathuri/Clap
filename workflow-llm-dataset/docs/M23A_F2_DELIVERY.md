...
4. **Variant comparison** — `compare_chain_runs(..., include_output_inventory=True, include_artifact_diff=False)`: `output_inventory_a/b` (step_id + output_paths per step), optional `artifact_diff` (only_in_a, only_in_b, common_count). CLI: `compare <a> <b>` or `--run-a A --run-b B`, plus `--artifact-diff`.
