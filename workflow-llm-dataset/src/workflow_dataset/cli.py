...
llm_group = typer.Typer(help="Local LLM training: corpus, SFT, train, eval, demo.")
...
setup_group = typer.Typer(help="Initial setup analyzer: long-running local onboarding.")
...
def setup_init(config: str = typer.Option("configs/settings.yaml", "--config", "-c")) -> None:
    ...
    if not setup_cfg.setup_scan_roots:
        console.print("[yellow]setup_scan_roots is empty; add paths in config to analyze[/yellow]")
    from workflow_dataset.setup.setup_manager import SetupManager
    setup_dir = Path(setup_cfg.setup_dir)
    graph_path = getattr(settings.paths, "graph_store_path", None) or "data/local/work_graph.sqlite"
    ...
    console.print(f"[green]setup session created: {session.session_id}[/green]")
    console.print(f"  onboarding_mode: {session.onboarding_mode}")
    console.print(f"  scan_roots: {session.scan_scope.root_paths[:5]}{'...' if len(session.scan_scope.root_paths) > 5 else ''}")
...
def setup_run(
    ...
    if not sid:
        ...
        if not sessions_dir.exists():
            console.print("[red]no session found; run 'setup init' first[/red]")
            raise typer.Exit(1)
    ...
    console.print(f"  files_scanned={progress.files_scanned} artifacts_classified={progress.artifacts_classified} docs_parsed={progress.docs_parsed}")
    console.print(f"  graph_nodes_created={progress.graph_nodes_created} current_stage={progress.current_stage.value}")
...
def setup_status(
    ...
    console.print(f"  style_patterns_extracted: {progress.style_patterns_extracted}")
    console.print(f"  graph_nodes_created: {progress.graph_nodes_created}")
    console.print(f"  adapter_errors: {progress.adapter_errors} adapter_skips: {progress.adapter_skips}")
...
def setup_summary(
    ...
    if not sid:
        console.print("[dim]no session; run 'setup init' and 'setup run' first[/dim]")
        raise typer.Exit(0)
    report_path = reports_dir / f"{sid}_summary.md"
    if not report_path.exists():
        console.print("[yellow]summary not generated yet; run 'setup run' to complete stages[/yellow]")
        raise typer.Exit(0)
    ...
    console.print(f"  personal_corpus: {'yes' if corpus_ok else 'no'} -> {corpus_path}")
    ...
    try:
        ...
        profiles_dir = Path(getattr(setup_cfg, "style_profiles_dir", "data/local/style_profiles"))
        suggestions_dir = Path(getattr(setup_cfg, "suggestions_dir", "data/local/suggestions"))
        draft_dir = Path(getattr(setup_cfg, "draft_structures_dir", "data/local/draft_structures"))
        profiles = load_style_profiles(profiles_dir) if profiles_dir.exists() else []
        candidates = collect_candidates_from_profiles(profiles_dir) if profiles_dir.exists() else []
        ...
        if profiles:
            console.print("  top profiles: " + ", ".join(p.profile_type for p in profiles[:3]))
        if drafts:
            console.print("  top drafts: " + ", ".join(d.draft_type for d in drafts[:3]))
    except Exception:
        pass
...
def setup_build_corpus(
    config: str = typer.Option("configs/settings.yaml", "--config", "-c"),
    session_id: str = typer.Option("", "--session-id"),
    output_dir: str = typer.Option("", "--output", "-o", help="Output dir; default data/local/llm/personal_corpus"),
) -> None:
    ...
    if not getattr(setup_cfg, "allow_personal_corpus_from_setup", False):
        console.print("[yellow]allow_personal_corpus_from_setup is false; enable in config to run[/yellow]")
        raise typer.Exit(0)
    sid = session_id or _resolve_latest_session_id(setup_cfg)
    if not sid:
        console.print("[yellow]no setup session; run 'setup init' and 'setup run' first[/yellow]")
        raise typer.Exit(0)
...
def setup_build_sft(
    config: str = typer.Option("configs/settings.yaml", "--config", "-c"),
    session_id: str = typer.Option("", "--session-id"),
    output_dir: str = typer.Option("", "--output", "-o", help="Output dir; default data/local/llm/personal_sft"),
) -> None:
    ...
    if not getattr(setup_cfg, "allow_personal_sft_from_setup", False):
        console.print("[yellow]allow_personal_sft_from_setup is false; enable in config to run[/yellow]")
        raise typer.Exit(0)
    sid = session_id or _resolve_latest_session_id(setup_cfg)
    if not sid:
        console.print("[yellow]no setup session; run 'setup init' and 'setup run' first[/yellow]")
    ...
    console.print(f"[green]personal SFT: train={n_train} val={n_val} test={n_test}[/green] -> {out_dir}")
...
def build_personal_corpus(
    ...
    session_id: str = typer.Option("", "--session-id"),
    output: str = typer.Option("", "--output", "-o", help="Output JSONL path; default data/local/llm/personal_corpus/personal_corpus.jsonl"),
) -> None:
    ...
    corpus_dir = Path(llm_cfg.get("corpus_path", "data/local/llm/corpus/corpus.jsonl")).parent
    if output:
        p = Path(output)
    ...
    if sid:
        ...
        if session_parsed.exists():
            ...
            console.print(f"[green]personal corpus written: {total} docs[/green] -> {out_path}")
            for k, v in counts.items():
                console.print(f"  {k}: {v}")
        else:
            console.print("[yellow]no parsed artifacts for session; run 'setup run' first[/yellow]")
    else:
        console.print("[yellow]no setup session; run 'setup init' and 'setup run' first[/yellow]")


# ----- Assistive loop (M5: style-aware suggestions and draft structures) -----
assist_group = typer.Typer(help="Style-aware suggestions and draft structures from setup. No execution.")
...
def assist_suggest(
    ...
    style_records = load_style_signals(sid, setup_cfg.style_signals_dir) if sid else []
    profiles = build_profiles_from_style_signals(style_records, session_id=sid or "", project_id="")
    ...
    if settings.agent and getattr(settings.paths, "event_log_dir", None):
        events = load_all_events(Path(settings.paths.event_log_dir), source_filter="file", max_events=2000)
        roots = [Path(r).resolve() for r in (settings.agent.file_observer.root_paths or [])] if settings.agent and settings.agent.file_observer else []
        routines = detect_routines(events, root_paths=roots if roots else None)
    suggestions = generate_style_aware_suggestions(context, existing_profiles, candidates, routines=routines)
    suggestions_dir = Path(getattr(setup_cfg, "suggestions_dir", "data/local/suggestions"))
    persist_style_aware_suggestions(suggestions, suggestions_dir)
    project_id_by_label = {p.get("label", ""): p.get("node_id", "") for p in context.get("projects", []) if p.get("label")}
    persist_style_profile_nodes(graph_path, existing_profiles, project_id_by_label)
    persist_imitation_candidate_nodes(graph_path, candidates, project_id_by_label)
    persist_style_aware_suggestion_nodes(graph_path, suggestions, project_id_by_label)
    console.print(f"[green]style-aware suggestions: {len(suggestions)}[/green] -> {suggestions_dir}")
    for s in suggestions[:5]:
        console.print(f"  [bold]{s.suggestion_type}[/bold] {s.title[:50]}")
        console.print(f"    [dim]{s.rationale[:80]}...[/dim]" if len(s.rationale) > 80 else f"    [dim]{s.rationale}[/dim]")
...
def assist_draft(
    ...
    for p in profiles:
        save_style_profile(p, profiles_dir)
    existing_profiles = load_style_profiles(profiles_dir)
    drafts = generate_draft_structures(context, existing_profiles, project_id="", domain_hint="")
    draft_dir = Path(getattr(setup_cfg, "draft_structures_dir", "data/local/draft_structures"))
    persist_draft_structures(drafts, draft_dir)
    project_id_by_label = {p.get("label", ""): p.get("node_id", "") for p in context.get("projects", []) if p.get("label")}
    persist_draft_structure_nodes(graph_path, drafts, project_id_by_label)
    console.print(f"[green]draft structures: {len(drafts)}[/green] -> {draft_dir}")
    for d in drafts[:5]:
        console.print(f"  [bold]{d.draft_type}[/bold] {d.title}")
        console.print(f"    [dim]sections: {', '.join(d.recommended_sections[:4])}[/dim]")
...
def assist_explain(
    query: str = typer.Argument(..., help="Question, e.g. 'explain this project', 'why did you suggest this?'"),
    config: str = typer.Option("configs/settings.yaml", "--config", "-c"),
    project_id: str = typer.Option("", "--project-id"),
    session_id: str = typer.Option("", "--session-id", help="Setup session ID (default: latest)"),
    ...
    resp = build_response(
        ...
        suggestions_dir=getattr(setup_cfg, "suggestions_dir", "data/local/suggestions"),
        draft_structures_dir=getattr(setup_cfg, "draft_structures_dir", "data/local/draft_structures"),
    ...
    if resp.supporting_evidence:
        console.print("[dim]Evidence: " + "; ".join(resp.supporting_evidence[:5]) + "[/dim]")
    console.print(f"[dim]confidence={resp.confidence_score:.2f} retrieval={resp.used_retrieval} llm={resp.used_llm}[/dim]")
...
def assist_refine_draft(
    draft_type: str = typer.Argument("", help="Draft type (e.g. project_brief, creative_brief_outline) or leave empty for first available"),
    config: str = typer.Option("configs/settings.yaml", "--config", "-c"),
    project_id: str = typer.Option("", "--project-id"),
    session_id: str = typer.Option("", "--session-id"),
    use_llm: bool = typer.Option(False, "--use-llm"),
...
def assist_chat(
    query: str = typer.Argument("", help="One-shot question, or leave empty for REPL"),
    config: str = typer.Option("configs/settings.yaml", "--config", "-c"),
    project_id: str = typer.Option("", "--project-id"),
    session_id: str = typer.Option("", "--session-id"),
    use_llm: bool = typer.Option(False, "--use-llm"),
    ...
    def run_turn(user_text: str) -> None:
        q = AgentQuery(
            query_id=stable_id("query", user_text[:50], utc_now_iso(), prefix="q"),
            user_text=user_text,
            project_id=project_id or getattr(al, "agent_loop_project_scope_default", "") or "",
            created_utc=utc_now_iso(),
        )
    ...
    if query:
        ...
        return
    # REPL
    console.print("[dim]Assist chat (REPL). Type a question and press Enter. Empty line or 'exit' to quit.[/dim]")
    while True:
        try:
            line = input("> ").strip()
        except EOFError:
...
def assist_materialize(
    draft_type: str = typer.Argument("", help="Draft type (e.g. project_brief, creative_brief_outline) or suggestion id"),
    config: str = typer.Option("configs/settings.yaml", "--config", "-c"),
    project_id: str = typer.Option("", "--project-id"),
    session_id: str = typer.Option("", "--session-id"),
    use_llm: bool = typer.Option(False, "--use-llm"),
    output_family: str = typer.Option("", "--output-family", help="text | table | folder_scaffold | creative_scaffold"),
    workspace: str = typer.Option("", "--workspace", help="Override workspace root"),
) -> None:
    ...
    sid = session_id or _resolve_latest_session_id(setup_cfg)
    graph_path = Path(getattr(settings.paths, "graph_store_path", "data/local/work_graph.sqlite"))
    workspace_root = Path(workspace or getattr(mat, "materialization_workspace_root", "data/local/workspaces"))
    corpus_path = Path("data/local/llm/personal_corpus/personal_corpus.jsonl")
    if not corpus_path.exists():
        corpus_path = None
    ...
    project_id_by_label = {p.get("label", ""): p.get("node_id", "") for p in (context_bundle.get("project_context") or {}).get("projects", []) if p.get("label")}
    llm_refine_fn = get_llm_refine_fn(llm_config_path=Path("configs/llm_training.yaml")) if use_llm else None
    ...
    if draft_type.startswith("sug_") or draft_type.startswith("suggestion_"):
        manifest, ws_path = materialize_from_suggestion(
            ...
            session_id=sid or "",
            project_id=project_id or "",
            save_manifests=getattr(mat, "materialization_save_manifests", True),
        )
    else:
        manifest, ws_path = materialize_from_draft(
            ...
            use_llm=use_llm,
            llm_refine_fn=llm_refine_fn,
            allow_markdown=getattr(mat, "materialization_allow_markdown", True),
            allow_csv=getattr(mat, "materialization_allow_csv", True),
            allow_json=getattr(mat, "materialization_allow_json", True),
            allow_folder_scaffolds=getattr(mat, "materialization_allow_folder_scaffolds", True),
            save_manifests=getattr(mat, "materialization_save_manifests", True),
        )
    ...
    console.print(f"[green]Materialized: {len(manifest.output_paths)} output(s)[/green] -> {ws_path}")
...
def assist_preview(
    workspace_path: str = typer.Argument(..., help="Path to materialized workspace (e.g. data/local/workspaces/materialized/req_xxx)"),
    config: str = typer.Option("configs/settings.yaml", "--config", "-c"),
) -> None:
    ...
    if manifest:
        console.print(render_preview(manifest, path, max_file_preview_chars=500))
    else:
...
def assist_list_workspaces(
    ...
    settings = load_settings(config)
    mat = _materialization_config(settings)
    root = Path(getattr(mat, "materialization_workspace_root", "data/local/workspaces"))
    ...
    if not items:
        ...
        raise typer.Exit(0)
    for w in items:
        console.print(f"  [bold]{w.get('name', '')}[/bold] -> {w.get('path', '')}")
...
def assist_apply_plan(
    workspace_path: str = typer.Argument(..., help="Path to sandbox workspace (e.g. data/local/workspaces/materialized/req_xxx)"),
    target_path: str = typer.Argument(..., help="Target directory to copy into"),
    config: str = typer.Option("configs/settings.yaml", "--config", "-c"),
    ...
    if not ok:
        ...
        raise typer.Exit(1)
    plan, err = build_apply_plan(ws, target, allow_overwrite=allow_overwrite, dry_run=True)
    if err and not plan:
...
def assist_apply(
    workspace_path: str = typer.Argument(..., help="Path to sandbox workspace"),
    target_path: str = typer.Argument(..., help="Target directory to copy into"),
    config: str = typer.Option("configs/settings.yaml", "--config", "-c"),
    confirm: bool = typer.Option(False, "--confirm", help="Explicit confirmation required to copy"),
    allow_overwrite: bool = typer.Option(False, "--allow-overwrite"),
) -> None:
    ...
    if not getattr(ap, "apply_enabled", False):
        console.print("[red]Apply is disabled. Enable apply.apply_enabled in config.[/red]")
        raise typer.Exit(1)
    if not confirm:
        console.print("[yellow]Apply requires explicit --confirm. Run without --confirm to see plan first.[/yellow]")
        raise typer.Exit(0)
    from workflow_dataset.apply.target_validator import validate_target
    from workflow_dataset.apply.copy_planner import build_apply_plan
    ...
    if getattr(ap, "apply_graph_persistence", True):
        graph_path = getattr(settings.paths, "graph_store_path", "data/local/work_graph.sqlite")
    ...
    console.print(f"[green]Applied {len(result.applied_paths)} path(s)[/green] -> {target}")
    if result.rollback_token:
        console.print(f"[dim]Rollback token: {result.rollback_token}[/dim]")
    console.print(f"[dim]Manifest: {manifest_root / 'results' / f'{result.result_id}.json'}[/dim]")


@assist_group.command("rollback")
def assist_rollback(
    rollback_token: str = typer.Argument(..., help="Rollback token from apply output"),
    config: str = typer.Option("configs/settings.yaml", "--config", "-c"),
) -> None:
...
def assist_apply_preview(
    ...
    from workflow_dataset.apply.copy_planner import build_apply_plan
    from workflow_dataset.apply.diff_preview import render_diff_preview
    plan, err = build_apply_plan(Path(workspace_path), Path(target_path), allow_overwrite=allow_overwrite, dry_run=True)
    if err and not plan:
        console.print(f"[red]{err}[/red]")
        raise typer.Exit(1)
    if plan:
...
def assist_generate_plan(
    ...
    project_id: str = typer.Option("", "--project-id"),
    session_id: str = typer.Option("", "--session-id"),
    generation_type: str = typer.Option("image_pack", "--type", "-t", help="image_pack, shot_plan, report_variant, design"),
    source_ref: str = typer.Option("", "--source-ref", help="draft_id or suggestion_id"),
    source_type: str = typer.Option("project", "--source-type", help="project, draft, suggestion"),
) -> None:
    ...
    request, manifest, ws_path = run_generation_plan(
        ...
        style_signals_dir=setup_cfg.style_signals_dir,
        parsed_artifacts_dir=setup_cfg.parsed_artifacts_dir,
        style_profiles_dir=getattr(setup_cfg, "style_profiles_dir", "data/local/style_profiles"),
        suggestions_dir=getattr(setup_cfg, "suggestions_dir", "data/local/suggestions"),
        draft_structures_dir=getattr(setup_cfg, "draft_structures_dir", "data/local/draft_structures"),
    ...
    console.print(f"  style_packs: {len(manifest.style_pack_refs)} prompt_packs: {len(manifest.prompt_pack_refs)} asset_plans: {len(manifest.asset_plan_refs)}")
    console.print(f"  workspace: {ws_path}")
    console.print(f"  manifest: {workspace_root / 'manifests' / (manifest.manifest_id + '.json')}")


@assist_group.command("generate-preview")
def assist_generate_preview(
    manifest_id: str = typer.Argument(..., help="Generation manifest ID (e.g. gm_...)"),
    config: str = typer.Option("configs/settings.yaml", "--config", "-c"),
    hydrate: bool = typer.Option(True, "--hydrate/--no-hydrate", help="Load prompt/asset/style packs from store"),
) -> None:
    ...
    if manifest.execution_records:
        ...
        for rec in manifest.execution_records:
            console.print(f"  backend: {rec.backend_name} v{rec.backend_version} status={rec.execution_status}")
            console.print(f"    used_llm={rec.used_llm} used_fallback={rec.used_fallback} executed_utc={rec.executed_utc}")
    ...
    if manifest.prompt_packs:
        for pp in manifest.prompt_packs[:5]:
            ...
            console.print(f"  [cyan]PromptPack {pp.prompt_family}[/cyan]: {txt}...")
    if manifest.asset_plans:
        for ap in manifest.asset_plans[:2]:
            console.print(f"  [cyan]AssetPlan[/cyan] targets: {ap.target_outputs[:5]}")
...
def assist_list_generations(
    ...
    items = list_generation_requests(root, session_id=session_id, project_id=project_id, limit=limit)
    if not items:
        console.print("[dim]No generation requests yet.[/dim]")
        raise typer.Exit(0)
    for g in items:
        console.print(f"  [bold]{g['generation_id']}[/bold] {g['generation_type']} session={g['session_id']} project={g['project_id']}")
...
def assist_generate_run(
    generation_id: str = typer.Argument(..., help="Generation ID from generate-plan"),
    config: str = typer.Option("configs/settings.yaml", "--config", "-c"),
    backend: str = typer.Option(None, "--backend", "-b", help="Backend: mock, document, image_demo (default from config)"),
    use_llm: bool = typer.Option(False, "--use-llm", help="Use local LLM when backend supports it"),
    allow_fallback: bool = typer.Option(True, "--allow-fallback/--no-fallback", help="Allow deterministic fallback"),
) -> None:
    ...
    if get_backend(backend) is None:
        console.print(f"[red]Unknown backend: {backend}. Use assist list-generation-backends.[/red]")
        raise typer.Exit(1)
    req_path = root / "requests" / f"{generation_id}.json"
    if not req_path.exists():
        console.print(f"[red]Generation request not found: {generation_id}[/red]")
        raise typer.Exit(1)
    ...
    if not manifest:
        console.print("[yellow]Manifest not found for this generation.[/yellow]")
        raise typer.Exit(0)
    load_packs_for_manifest(manifest, root)
    ...
    manifest.generated_output_paths = list(manifest.generated_output_paths) + output_paths
    if execution_record:
        manifest.execution_records = list(manifest.execution_records) + [execution_record]
...
def assist_list_generation_backends(config: str = typer.Option("configs/settings.yaml", "--config", "-c")) -> None:
    ...
    for meta in list_backends():
        console.print(f"  [bold]{meta.backend_name}[/bold] ({meta.backend_type}) v{meta.version}")
...
def assist_generate_review(
    ...
    config: str = typer.Option("configs/settings.yaml", "--config", "-c"),
    manifest_id: str = typer.Option("", "--manifest-id", "-m", help="Optional manifest ID to load paths from"),
    compare: bool = typer.Option(False, "--compare", help="List variants for comparison"),
) -> None:
    ...
    if not getattr(gen, "generation_preview_enabled", True) or not getattr(gen, "generation_review_enabled", True):
        console.print("[yellow]Generation review is disabled in config.[/yellow]")
        raise typer.Exit(0)
    root = Path(getattr(gen, "generation_workspace_root", "data/local/generation"))
    workspace_path = root / generation_id
    ...
    if not manifest_id:
        ...
        manifest = load_generation_manifest(manifest_id, root)
    if not manifest:
        console.print("[yellow]Manifest not found; listing workspace files.[/yellow]")
        paths = [str(f) for f in workspace_path.rglob("*") if f.is_file()] if workspace_path.exists() else []
    else:
        load_packs_for_manifest(manifest, root)
    ...
    if compare:
        ...
        if variants:
            ...
            for v in variants[:10]:
                console.print(f"  {v.variant_id} {v.variant_type} {v.revision_note[:40]}...")
        else:
...
def assist_generate_refine(
    generation_id: str = typer.Argument(..., help="Generation ID"),
    artifact_path: str = typer.Argument(..., help="Path to generated artifact (relative to generation workspace or absolute)"),
    config: str = typer.Option("configs/settings.yaml", "--config", "-c"),
    use_llm: bool = typer.Option(False, "--use-llm", help="Use local LLM refinement if enabled"),
    instruction: str = typer.Option("", "--instruction", "-i", help="Optional refinement instruction"),
) -> None:
    ...
    llm_fn = get_llm_refine_fn_for_review(Path("configs/llm_training.yaml")) if use_llm else None
...
def assist_generate_adopt(
    generation_id: str = typer.Argument(..., help="Generation ID"),
    paths: str = typer.Argument(..., help="Comma-separated relative paths to adopt (e.g. creative_brief_generated.md)"),
    config: str = typer.Option("configs/settings.yaml", "--config", "-c"),
    ...
    console.print(f"[green]Adoption candidate created: {candidate.adoption_id}[/green]")
    console.print("  workspace:", candidate.workspace_path)
    console.print("  paths:", candidate.candidate_paths)
    console.print("[dim]To apply: use apply-plan with this workspace and target path, or use console Apply flow.[/dim]")
...
def assist_generate_compare(
    ...
    console.print(f"Path A: {result.get('path_a')}  lines={result.get('line_count_a')}  size={result.get('size_a')}")
    console.print(f"Path B: {result.get('path_b')}  lines={result.get('line_count_b')}  size={result.get('size_b')}")
    console.print(f"Same content: {result.get('same_content')}")
    if result.get("preview_a"):
        console.print(Panel(result["preview_a"][:1500], title="Preview A", border_style="blue"))
    if result.get("preview_b"):
        console.print(Panel(result["preview_b"][:1500], title="Preview B", border_style="green"))
...
def assist_bundle_create(
    adapter_type: str = typer.Argument(..., help="Adapter: spreadsheet, creative_package, design_package, ops_handoff"),
    config: str = typer.Option("configs/settings.yaml", "--config", "-c"),
    ...
    """Create a toolchain-native output bundle from an adapter. Sandbox-only."""
    settings = load_settings(config)
    ...
    if get_adapter(adapter_type) is None:
        console.print(f"[red]Unknown adapter: {adapter_type}. Use assist list-bundles to see adapters.[/red]")
        raise typer.Exit(1)
    bundle_root = Path(getattr(oa, "output_adapter_bundle_root", "data/local/bundles"))
    bundle_root.mkdir(parents=True, exist_ok=True)
    ts = utc_now_iso()
    req_id = stable_id("adreq", adapter_type, generation_id or review_id or ts, prefix="adreq")
...
def assist_bundle_adopt(
    ...
    if not workspace_path.exists():
        console.print(f"[red]Bundle workspace not found: {workspace_path}[/red]")
        raise typer.Exit(1)
    candidate = create_adoption_candidate(
    ...
    review_root = Path(getattr(settings.generation, "generation_workspace_root", "data/local/generation")) / "review" if getattr(settings, "generation", None) else bundle_root / "review"
...
def assist_list_bundles(
    ...
    for b in list_bundles(bundle_root, limit=limit):
        console.print(f"  {b['bundle_id']}  adapter={b['adapter_used']}  {b['created_utc'][:10]}")
...
def llm_prepare_corpus(
    ...
    if total > 0:
        ...
        if preview:
            console.print("[dim]Preview (first doc):[/dim]", preview[0].title[:60] + "...")
...
def llm_build_sft(
    ...
    console.print(f"[green]SFT split: train={n_train} val={n_val} test={n_test}[/green] -> {sft_dir}")
    for task_type, c in counts.items():
        console.print(f"  {task_type}: {c}")
...
def llm_train(
    ...
    if not (sft_dir / "train.jsonl").exists():
        console.print("[red]SFT train.jsonl not found; run 'llm build-sft' first[/red]")
        raise typer.Exit(1)
    ...
    try:
        ...
        console.print(f"[green]training complete; adapter -> {adapter_path}[/green]")
...
def llm_eval(
    ...
    """Evaluate on test split. Outputs metrics.json, predictions.jsonl, eval_summary.md."""
    llm_cfg = _load_llm_config(llm_config)
    sft_dir = Path(llm_cfg.get("sft_train_dir", "data/local/llm/sft"))
    ...
    if not test_file.exists():
        ...
        raise typer.Exit(0)
    from workflow_dataset.llm.eval import run_eval
    runs_dir = Path(llm_cfg.get("runs_dir", "data/local/llm/runs"))
    ...
    console.print(f"[green]eval done: {result.num_examples} examples[/green] -> {result.predictions_path}")
...
def llm_demo(
    ...
    retrieve: int = typer.Option(0, "--retrieve", "-r", help="Top-k corpus docs to retrieve and show as context"),
) -> None:
    """Single-prompt demo. Optionally retrieve top-k corpus docs as context."""
    llm_cfg = _load_llm_config(llm_config)
    corpus_path = llm_cfg.get("corpus_path", "data/local/llm/corpus/corpus.jsonl")
    if retrieve > 0 and Path(corpus_path).exists():
        ...
        docs = do_retrieve(corpus_path, prompt or "workflow occupation", top_k=retrieve)
    ...
    if prompt:
        model_path = llm_cfg.get("base_model", "")
        if not model_path:
            console.print("[dim]No base_model in config; skipping generation[/dim]")
            raise typer.Exit(0)
        try:
            result = subprocess.run(
                [sys.executable, "-m", "mlx_lm.generate", "--model", model_path, "--prompt", prompt, "--max-tokens", "150"],
                capture_output=True,
                text=True,
                timeout=60,
            )
            if result.returncode == 0:
                ...
                console.print(result.stdout or "[no output]")
            else:
                console.print("[yellow]mlx_lm.generate not available or failed[/yellow]")
        except Exception as e:
            console.print(f"[yellow]demo generation failed: {e}[/yellow]")
    else:
        console.print("Use --prompt 'your question' to run a single prompt. Use --retrieve 3 to show top-3 corpus context.")
