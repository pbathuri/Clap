...
ACTIONS = ("build", "benchmark", "cohort_test", "promote", "hold", "rollback", "replay_task", "observe_setup")
...
def recommend_next_action(state: dict[str, Any]) -> dict[str, Any]:
    ...
    # Observation (M31): no sources enabled — suggest enabling file observation
    obs = state.get("observation_state", {})
    if not obs.get("error") and not obs.get("enabled_sources"):
        return {
            "action": "observe_setup",
            "rationale": "Observation disabled; enable at least one source for personal agent input.",
            "detail": "observe enable --source file; set agent.file_observer.root_paths in config; then observe run.",
        }
