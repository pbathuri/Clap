...
def run_job(
    ...
    job = get_job_pack(job_pack_id, root)
    if not job:
        return {"error": f"Job pack not found: {job_pack_id}", "job_pack_id": job_pack_id, "resolved_behavior": {}}
    spec = load_specialization(job_pack_id, root)
    resolved = resolve_params(job, spec.preferred_params, params)
    allowed, msg = check_job_policy(job, mode, resolved, root)
    # M25E–M25H: Resolve pack behavior for this job (inspectable; used by runners that consume result)
    resolved_behavior: dict[str, Any] = {}
    try:
        from workflow_dataset.packs.behavior_runtime import get_behavior_summary_for_job
        resolved_behavior = get_behavior_summary_for_job(job_pack_id, root)
    except Exception:
        pass

    if not allowed:
        return {"error": msg, "job_pack_id": job_pack_id, "resolved_behavior": resolved_behavior}

    source = job.source
    if not source:
        return {"error": f"Job {job_pack_id} has no source (task_demo or benchmark_case).", "job_pack_id": job_pack_id, "resolved_behavior": resolved_behavior}

    if source.kind == "task_demo":
        if mode == "real":
            return {"error": "Task demo replay is simulate-only.", "job_pack_id": job_pack_id, "resolved_behavior": resolved_behavior}
        task, results = replay_task_simulate(source.ref, root)
        if not task:
            return {"error": f"Task not found: {source.ref}", "job_pack_id": job_pack_id, "resolved_behavior": resolved_behavior}
    ...
    if source.kind == "benchmark_case":
        ...
        if bench_result.get("error"):
            return {"error": bench_result["error"], "job_pack_id": job_pack_id, "resolved_behavior": resolved_behavior}
    ...
    return {"error": f"Unsupported source kind: {source.kind}", "job_pack_id": job_pack_id, "resolved_behavior": resolved_behavior}
