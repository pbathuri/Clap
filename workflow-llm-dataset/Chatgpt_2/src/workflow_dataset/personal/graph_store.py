...
def init_store(db_path: Path) -> None:
    ...
    try:
        ...
        """)
        conn.execute("""
            CREATE TABLE IF NOT EXISTS graph_review (
...
def list_edges(
    ...
    """List edges with optional filters. Returns list of dicts: from_id, to_id, relation_type."""
    conditions = []
    params: list[Any] = []
    ...
    params.append(limit)
    rows = conn.execute(
        f"SELECT from_id, to_id, relation_type FROM edges{where} LIMIT ?",
        params,
    ).fetchall()
    return [{"from_id": r[0], "to_id": r[1], "relation_type": r[2]} for r in rows]
...
def save_graph_review_item(
    ...
    """Insert or replace a graph review item (routine confirmation or pattern review)."""
    payload_json = json.dumps(payload) if payload else "{}"
    conn.execute(
        """
        INSERT OR REPLACE INTO graph_review (item_id, kind, payload, status, created_utc, updated_utc)
        VALUES (?, ?, ?, ?, ?, ?)
        """,
        (item_id, kind, payload_json, status, created_utc or "", updated_utc or ""),
    )


def get_graph_review_item(conn: sqlite3.Connection, item_id: str) -> dict[str, Any] | None:
    """Load one graph review item by id."""
    row = conn.execute(
        "SELECT item_id, kind, payload, status, created_utc, updated_utc FROM graph_review WHERE item_id = ?",
        (item_id,),
    ).fetchone()
    if not row:
        return None
    ...
    return {
        ...
        "payload": payload,
        "status": row[3],
        "created_utc": row[4] or "",
        "updated_utc": row[5] or "",
    }


def list_graph_review_items(
    conn: sqlite3.Connection,
    kind: str | None = None,
    status: str | None = None,
    limit: int = 100,
) -> list[dict[str, Any]]:
    """List graph review items, optionally filtered by kind and status."""
    conditions = []
    params: list[Any] = []
    if kind:
    ...
    params.append(limit)
    rows = conn.execute(
        f"SELECT item_id, kind, payload, status, created_utc, updated_utc FROM graph_review{where} ORDER BY created_utc DESC LIMIT ?",
        params,
    ).fetchall()
    out = []
    for r in rows:
...
def update_graph_review_status(
    conn: sqlite3.Connection,
    item_id: str,
    status: str,
    updated_utc: str = "",
    ...
    """Update status (and optionally payload) of a graph review item. Returns True if row was updated."""
    if payload_update is not None:
        row = conn.execute("SELECT payload FROM graph_review WHERE item_id = ?", (item_id,)).fetchone()
        if not row:
            return False
        payload = json.loads(row[0]) if row[0] else {}
        payload.update(payload_update)
        conn.execute(
            "UPDATE graph_review SET status = ?, updated_utc = ?, payload = ? WHERE item_id = ?",
            (status, updated_utc or "", json.dumps(payload), item_id),
        )
    else:
        conn.execute(
            "UPDATE graph_review SET status = ?, updated_utc = ? WHERE item_id = ?",
            (status, updated_utc or "", item_id),
        )
