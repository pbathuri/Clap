...
def build_trust_cockpit(repo_root: Path | str | None = None) -> dict[str, Any]:
    ...
    # M23Q: Release gate checks and safe_to_expand (advisory only)
    try:
        from workflow_dataset.trust.release_gates import evaluate_release_gates, safe_to_expand
        out["release_gate_checks"] = evaluate_release_gates(out)
        expand = safe_to_expand(out)
        out["safe_to_expand"] = expand.get("safe", False)
        out["safe_to_expand_reasons"] = expand.get("reasons", [])
        out["failed_gates"] = expand.get("failed_gates", [])
    except Exception as e:
        out["errors"].append(f"release_gates: {e}")
        out["release_gate_checks"] = []
        out["safe_to_expand"] = False
        out["safe_to_expand_reasons"] = [str(e)]
        out["failed_gates"] = []
