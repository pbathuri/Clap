...
def format_release_gates(cockpit: dict[str, Any] | None = None, repo_root=None) -> str:
    ...
    checks = cockpit.get("release_gate_checks") or []
    if checks:
        lines.append("[Gate checks]")
        for c in checks:
            status = "PASS" if c.get("passed") else "FAIL"
            lines.append(f"  {status}  {c.get('name', '')} — {c.get('message', '')}")
        lines.append("")
    safe = cockpit.get("safe_to_expand", False)
    lines.append(f"Safe to expand (advisory): {'yes' if safe else 'no'}")
    return "\n".join(lines)


def format_readiness_report(cockpit: dict[str, Any] | None = None, repo_root=None) -> str:
    """M23Q: Operator-facing readiness: is the system safe to expand? Advisory only."""
    if cockpit is None:
        cockpit = build_trust_cockpit(repo_root)
    lines = [
    ...
    safe = cockpit.get("safe_to_expand", False)
    reasons = cockpit.get("safe_to_expand_reasons") or []
    lines.append(f"Safe to expand: {'YES' if safe else 'NO'}")
    lines.append("")
    for r in reasons:
        lines.append(f"  • {r}")
    lines.append("")
    lines.append("[Release gate checks]")
    for c in cockpit.get("release_gate_checks") or []:
        status = "PASS" if c.get("passed") else "FAIL"
        lines.append(f"  {status}  {c.get('name', '')} — {c.get('message', '')}")
    lines.append("")
    bt = cockpit.get("benchmark_trust") or {}
    lines.append("[Summary]")
    lines.append(f"  Benchmark: {bt.get('latest_run_id') or 'no run'}  outcome: {bt.get('latest_outcome') or 'n/a'}  regressions: {len(bt.get('regressions') or [])}")
    ar = cockpit.get("approval_readiness") or {}
    lines.append(f"  Approval registry: {'present' if ar.get('registry_exists') else 'missing'}")
    uc = cockpit.get("unresolved_corrections") or {}
    lines.append(f"  Proposed corrections: {uc.get('proposed_updates_count', 0)}  review recommended: {len(uc.get('review_recommended_ids') or [])}")
    return "\n".join(lines)
